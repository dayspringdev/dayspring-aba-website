...
// A generic data type for our templates.
// We use `any` here for flexibility, as each template has different data needs.
// In a larger system, you could create specific types for each template's data.
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif, 'Apple Color Emoji', 'Segoe UI Emoji', 'Segoe UI Symbol'; background-color: #f8f9fa;">
  ${preheader ? `<span style="display: none !important;">${preheader}</span>` : ""}
  ...
  adminNewBookingNotice: {
    ...
    body: (data) => {
      ...
      const content = `
        ...
        <ul style="font-size: 16px; color: #495057; line-height: 1.6; list-style-type: none; padding: 0;">
  ...
  bookingConfirmed: {
    ...
    body: (data) => {
      ...
      const content = `
        ...
        <p style="font-size: 16px; color: #495057; line-height: 1.6;">
          We look forward to speaking with you. If you have any questions before then, please don't hesitate to reach out.
        </p>
        <p style="font-size: 16px; color: #495057; line-height: 1.6;">Sincerely,<br>The Dayspring Team</p>
      `;
      return baseTemplate(title, content, preheader);
  ...
  contactFormToBusiness: {
    ...
    body: (data) => {
      ...
      const content = `
          ...
          <ul style="font-size: 16px; color: #495057; line-height: 1.6; list-style-type: none; padding: 15px; border-left: 4px solid #eee;">
            <li><strong>Name:</strong> ${data.firstName} ${data.lastName}</li>
            <li><strong>Email:</strong> ${data.email}</li>
            <li><strong>Phone:</strong> ${data.phone || "Not provided"}</li>
          </ul>
          <h3 style="font-size: 18px; color: #212529; margin-top: 20px;">Message:</h3>
          <p style="font-size: 16px; color: #495057; line-height: 1.6; padding: 15px; border-left: 4px solid #eee; white-space: pre-wrap;">${data.message}</p>
        `;
      return baseTemplate(title, content);
    },
  },
